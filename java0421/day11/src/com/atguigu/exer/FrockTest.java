...
/*
在Frock类中声明私有的静态属性currentNum，初始值为100000，
作为衣服出厂的序列号起始值。
声明公有的静态方法getNextNum，作为生成上衣唯一序列号的方法。每调用一次，
...
在TestFrock类的main方法中，分别创建三个Frock 对象，并打印三个对象的序列号，
验证是否为按100递增。

在Frock类中声明静态语句块，语句块中将currentNum的初始值设为150000，
作为衣服出厂的序列号起始值，并打印输出该值。
执行TestFrock类的main方法，分别创建三个Frock 对象，
验证静态语句块是否只执行一次，以及序列号起始值是否已调整。
